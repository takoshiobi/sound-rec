...
def getBatch(data, batchSize, iteration):
    ...
    :return: выборка подходящих примеров из пакета c данными, необходимых для обучения нейронки
    :rtype: массив
    """
    # начало серии 
    # пример
...
# placeholder: местозаполнитель для тензора
...
Функция SoftMax, по-русски функция мягкого максимума, часто используется в нейронных сетях 
в качестве функции активации при решении задачи классификации.
"""
...
# объект Session инкапсулирует среду, в которой выполняются классы операций, и оцениваются объекты тензоров.
with tf.Session() as sess:
    # initialise the variables
    # метод run запускает операции и оценивает тензоры в выборках.
    sess.run(init_op)

    # восстанавливает ранее созданную и сохраненную в файле model.py модель
    ...
    for epoch in range(num_of_epochs):
        ...
        # feed_dict это словарь, ассоциирующий элементы графа со значениями
        # (y_) это один элемент графа, список элементов графа или словарь, значения которого являются элементами графа или списками графа
        ...
        # softmax позволяет трактовать выходные значения нейронов как вероятность принадлежности данному классу
        # переменная predictions выводит вектор, который представляет распределения вероятностей списка потенциальных результатов.
        if epoch == 0:
